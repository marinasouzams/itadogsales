...
export default function RepTarefas() {
  const { user } = useAuth()
  const { data: rawTasks, loading } = useTasks({ assignedTo: user?.id })
  ...
  return (
    <RepLayout title="Minhas Tarefas">
      <div className="p-4 space-y-4 max-w-full pb-24">
          ...
          <>
            ...
            >
              ...
              <div className="p-4 space-y-4">
                ...
                {/* Responsável fixo para rep */}
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Responsável</label>
                  <div className="w-full text-sm border border-slate-100 rounded-lg px-3 py-2.5 bg-slate-50 text-slate-500">
                    {user?.name || 'Você'}
                  </div>
                </div>
